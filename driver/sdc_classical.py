...
def do_MD(init_coords,types,symbols,latticeVectors,nl,nlTrX,nlTrY,nlTrZ, init_vels, dt, num_steps, gamma=0.1):
    ...
    for i in range(2): symbols[i] = "C"
    p=ptable()
    atomicnumbers=np.zeros(nats,dtype=int)
    for i in range(nats): atomicnumbers[i]=p.get_atomic_number(symbols[types[i]])
    ...
    for i in range(0,num_steps):
        ...
        forces = get_sdc_classical_forces(coords,types,symbols,latticeVectors,nl,nlTrX,nlTrY,nlTrZ,"HarmonicAll",verb=False)
