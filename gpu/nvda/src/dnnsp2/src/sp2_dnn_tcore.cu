...
void 
build_H0(const unsigned N, float *X) {
    for(int i=0; i<N; ++i) {
        for(int j=i; j<N; ++j) {
...
void        // only writes upper triangle of D0 and D1
save_DM(
  const unsigned N,
  double *X0,
  double *X1) {

  std::ofstream file_D0 ("D0.dat");
  std::ofstream file_D1 ("D1.dat");
  std::cout << N << std::endl;
  if (file_D0.is_open())
  {
    file_D0 << "Matrix of size N = " << N << "\n" ;
    file_D1 << "Matrix of size N = " << N << "\n" ;
    for(int i=0; i<N; ++i) {
        for(int j=i; j<N; ++j) {
            file_D0 << X0[i*N + j] << "\n" ;
            file_D1 << X1[i*N + j] << "\n" ;
...
int main(int argc, char *argv[])
{
    ...
    std::cout << N << " " << TFLOPS << " TFLOPS" <<std::endl;
