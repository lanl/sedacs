...
def build_nlist_vector(coords,latticeVectors,rcut,rank=0,numranks=1,verb=False):
...
#        iz =  int((coords[i,2] - minz + smallReal)/(2.0*rcut)) #small box z-index
    ...
    def get_neighs_of(i,boxOfI,inbox,totPerBox,latticeVectors):    
        #print("atom",i)
        cnt = -1
        #Get the list of all atoms in neighboring boxes
        boxneighs = inbox[neighbox[boxOfI[i]]]
...
#        dvec = dvec + coords[None,None,i]
    ...
    def get_neighs_of_range(i0,i1,coords,boxOfI,inbox,latticeVectors):    
        #print("atom",i)
        cnt = -1
        nats_this = i1 - i0 + 1
        #Get the list of all atoms in neighboring boxes
...
#        dvec = dvec + coords[None,None,i]
        orthovec = np.diagonal(latticeVectors)
        tic = time.perf_counter()
        repeats = np.repeat(boxneighs.shape[1]*boxneighs.shape[2],nats_this)
...
#        i = firstIdx + k
        nlVect = get_neighs_of(i,boxOfI,inbox,totPerBox,latticeVectors)
        nlChunk[k,:] = nlVect[:] 
