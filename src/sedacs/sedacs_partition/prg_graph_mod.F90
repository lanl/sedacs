...
!    gp%myRank = getMyRank()
  ...
  !!
  subroutine prg_sedacsPartition(whichParts_guess_saved,graph,degs,nparts,nnodes,verb)
    implicit none
    ...
    integer ::  maxDeg, nodesPerPart,toBeDistributed,sumSizes,rem
    integer, allocatable :: whichParts(:)
    integer, allocatable :: graph(:,:),degs(:)
    real(kind(1.0)), allocatable :: row(:)
    real(dp) :: bal,sumDegs, avcut, avcutOld
    character(len=100) :: pname
    ...
    maxDeg = size(graph(0, :))
    call bml_zero_matrix('ellpack',bml_element_real,dp,nnodes,nnodes,g_bml)
    allocate(row(nnodes))
    do i = 1,nnodes
      row = 1.0_dp
      do j = 1,degs(i)
        row(graph(i,j)) = 1.0_dp
      enddo
    ...
    avcutOld = 0.0_dp
    do i = 1,200
      call do_flips_precomp(whichParts,graph,g_bml,degs,nnodes,nparts,bal)
      call prg_get_graphCut(whichParts,degs,graph,cut)
      call prg_get_balancing(whichParts,nparts,bal)
      if (i .eq. 1) avcut = cut
      avcut = (avcut + real(cut, dp)) / 2.0_dp
      if(writeOut) write(*,*)"Iter, Cut, RelCut, AvCut, and Bal",i,cut, cut/sumDegs,avcut,bal
      if (avcut .ge. avcutOld .and. i .gt. 3)then
        exit
      endif
      avcutOld = avcut
    enddo
  ...
  !
  subroutine do_flips_precomp(whichParts,graph,adj,degs,nnodes,nparts,bal)
    implicit none
