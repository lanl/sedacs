...
def get_kernel_byParts(
    sdc, rank, numranks, parts, partsCoreHalo, sy, mu=0.0, device="cuda", dtype=torch.float64
):
    """
    ...
    atomtypes = torch.from_numpy(sy.types).to(device).to(torch.int32)
    ...
    for partIndex in range(partIndex1, partIndex2):
        ...
        tmpArray = torch.from_numpy(subSy.orbs[subSy.types[0:numberOfCoreAtoms]]).to(device).to(torch.int32)
        ...
        # Get the hindex 
        hindex = torch.from_numpy(subSy.hindex).to(device).to(torch.int32)
        ...
        for i in range(numberOfCoreAtoms):
            ...
            coulvs, ewald_e = get_PME_coulvs(
                chargePertVect, hubbard_u, coords, atomtypes, lattice_vecs, device=device, use_torch=True 
            )
            ...
            indices = torch.arange(hindex[0], hindex[-1], device=device, dtype=torch.int32)
            ...
            pt_charges.index_add_(0, index, fullDiag) # same as code below
            # for j in range(subSy.nats):
            #    pt_charges[j] = 0.0
...
def rankN_update_byParts(
    q_n, n, maxRanks, sdc, rank, numranks, comm, parts, partsCoreHalo, sy, mu=0.0, device="cuda", dtype=torch.float64
):
    """
    ...
    if is_mpi_available and numranks > 1:
        K0Res = collect_and_sum_vectors_float(K0ResOnRank.cpu().numpy(), rank, numranks, comm)
        K0Res = torch.from_numpy(K0Res).to(device)
    ...
    while irank < mRank - 1 and error > 1e-6:
        ...
        if is_mpi_available and numranks > 1:
            trP1 = collect_and_sum_vectors_float(trP1.cpu().numpy(), rank, numranks, comm)
            trP1 = torch.from_numpy(trP1).to(device)
            trdPdMu = collect_and_sum_vectors_float(trdPdMu.cpu().numpy(), rank, numranks, comm)
            trdPdMu = torch.from_numpy(trdPdMu).to(device)
            comm.Barrier()
        ...
        if is_mpi_available and numranks > 1:
            dr = collect_and_sum_vectors_float(dr.cpu().numpy(), rank, numranks, comm)
        ...
        if is_mpi_available and numranks > 1:
            c_i_temp = collect_and_sum_vectors_float(c_i_temp.cpu().numpy(), rank, numranks, comm)
            c_i_temp = torch.from_numpy(c_i_temp).to(device)
            comm.Barrier()
        c_i[irank] = c_i_temp
        ...
        if is_mpi_available and numranks > 1:
            auxVect = collect_and_sum_vectors_float(auxVect.cpu().numpy(), rank, numranks, comm)
            auxVect = torch.from_numpy(auxVect).to(device)
            comm.Barrier()
