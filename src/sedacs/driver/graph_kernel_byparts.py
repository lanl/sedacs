...
def get_kernel_byParts(
    ...
    for partIndex in range(partIndex1, partIndex2):
        ...
        for i in range(numberOfCoreAtoms):
            ...
            H_dq_v = 0.5 * (np.matmul(subSy.over, H_dq_v) + np.matmul(H_dq_v, subSy.over.T)) 
...
def rankN_update_byParts(
    ...
    for irank in range(maxRanks):
        ...
        if irank > 0:
            for kk in range(irank):
                vi[:, irank] = vi[:, irank] - np.dot(vi[:, kk], vi[:, irank]) * vi[:, kk]
        ...
        for partIndex in range(partIndex1, partIndex2):
            ...
            f[:] = q1[:, partIndex - partIndex1] - v_core_i[:, partIndex - partIndex1, irank]
    ...
    #         for partIndex in range(partsPerRank):
    #             auxVect[i * maxRanks + j] = auxVect[i * maxRanks + j] + np.dot(ff[:, partIndex, i], ff[:, partIndex, j])
