...
def generate_H(N):
    ...
    for i in range(0, N):
        for j in range(i, N):
            H[i, j] = np.exp(-0.5 * np.abs(i - j)) * np.sin(i + 1)
            H[j, i] = H[i, j]
...
if __name__ == "__main__":  # execute main script only when directly called from the command line
    ...
    parser = argparse.ArgumentParser(
        description="Input N, number of electrons, and Nocc, the occupation number.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    parser.add_argument("-n", dest="N", type=int, help="number of electrons", required=True)
    parser.add_argument("-nocc", dest="Nocc", type=int, help="number of occupied orbitals", required=True)
    ...
    H0 = np.asarray([[5, -4, 3, 2, 1], [-4, 5, 3, 2, -1], [3, 3, 3, 2, 0], [2, 2, 2, 2, 1], [1, -1, 0, 1, 3]])
    X0 = H0  # Initial input layer
    # H1 = 1.0*I                   # Initial input layer
    H1 = np.asarray([[3, 1, 2, 1, 0], [1, 1, 2, 2, 0], [2, 2, 5, 3, 1], [1, 2, 3, 7, 0], [0, 0, 1, 0, 9]])
    # H1[0,0] = .001
    # H1[1,1] = -0.001
    X1 = H1
    ...
    e, v = np.linalg.eig(X0)  # Diagonlize H as a brute force comparision
    ...
    for layer in range(maxlayer):
        ...
        print(
            layer, "0th order Idemp error estimate:" + str(idemp_err[layer])
        )  # Can reach 0 exactely in low precision arithmetics
        print(
            layer, "1st order Idemp error estimate:" + str(idemp_err_1[layer])
        )  # Can reach 0 exactely in low precision arithmetics
        print("1st order Occupation error:" + str(np.trace(S1)))
    ...
    if Refinement:
        ...
        X0 = 2 * X0 - X02
        TrS0 = np.trace(X0)
        X0 = np.double(np.matmul(X0, X0))
        TrX0 = np.trace(X0)
        idemp_err[layer + 1] = TrS0 - TrX0
        print(layer + 1, "Refined 0th error estimate = " + str(idemp_err[layer + 1]))
        num_deep_layers = layer + 1  # +1 to account for the last refinement layer
        D0 = X0  # Output layer estimate of density matrix

        X0X1 = np.matmul(X0, X1)
        X0X1X1X0 = X0X1 + np.transpose(X0X1)
        TrS1 = np.trace(X0X1X1X0)
        D1 = 2 * X0X1X1X0 - np.matmul(X02, X0X1X1X0) - np.matmul(X0X1X1X0, X02)
        print(layer + 1, "Refined 1st order error estimate = " + str(TrS1 - np.trace(D1)))
    ...
    print("1st order Occupation error:" + str(np.trace(D1)))
    ...
    print(" E0 + E1 + E2 = " + str(energy + energy_1 + energy_2))
    # print(D1)
