...
__global__ void buildId_dev_kernel(double *Iden, int n)
{
...
__global__ void cheby_coeffs_dev_kernel(const double emax, const double emin,
                                        const double ef,
                                        const double kbt,
                                        const int npts,
                                        const int K, const int M,
...
void ps_cheby_coeffs_dev(double *ps_c, double *c, const int K, const int M, cublasHandle_t handle)
{
    ...
    // follow Eq 20
    for (int block_i = M - 1; block_i > -1; block_i--)
    { // go backwards, start filling from lower-left corner of X
        for (int block_j = M - 1; block_j > block_i - 1; block_j--)
        { // go backwards, enforce upper-triangularity
