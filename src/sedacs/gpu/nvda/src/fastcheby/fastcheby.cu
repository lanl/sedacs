...
void pscheby(double *d_ham, double *d_dm,
             int K, int M,
             int N,
             double mu,
             double kbt)
{
    ...
    gershgorin_v2(N, d_ham, &h1, &hN, streamm);
    ...
    // initialize T1 = H
    CUDA_CHECK_ERR(cudaMemcpy(d_T[1], d_ham, N * N * sizeof(double), cudaMemcpyDeviceToDevice));
    ...
    CUDA_CHECK_ERR(cudaMemcpy(d_dm, d_aux[M - 1], N * N * sizeof(double), cudaMemcpyDeviceToDevice));
};
