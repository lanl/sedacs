               ...
               void *Handle)
{
    ...
    // compute H-mu*I and mu to zero
    float minus_mu = float(-mu);
    std::cout << "minus mu = " << minus_mu << std::endl;
    ...
    float h1, hN;
    gershgorin_v2<float>(N, d_S0, &h1, &hN);

    //printf("%f, %f, %f \n",h1, hN,mu);

    // input layer to Golden SP2
    hN=max(abs(hN),abs(h1));

    // map spectrum to (0,1) and mu to 1/phi
//    rho[:,:] = ((emax * ident[:,:] - rho[:,:]) / (emax - emin)) * ((emax-emin)/emax)*inv_golden*inv_golden #if inv_golden, need to use x^2 first, otherwise

    float inv_golden = 0.61803398875;    //\varphi^{-1}=\varphi-1

    // zeroth-order term
    float mu_factor = ((hN-h1)/hN)*inv_golden ;
    float a = (hN / (hN - h1)) * mu_factor;
    float b = (-1 / (hN - h1)) * mu_factor;
    ...
    nvtxRangePop();
    //double mu_temp = float(mu);
    mu = ((hN-0)/(hN-h1))*mu_factor;
