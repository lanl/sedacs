...
#
##
def build_HS(nats,coords_in,box,elempointer,basis,hes,hep,\
        hed,hef,hdim,cutoffList,iglMap,tabh,tabs,sspl,hspl,lentabint,tabr,hcut,scut,verbose):
    if(verbose):
        print("In build_HS ...")
                                ...
                                for mbra in range(-lbra,lbra+1):
                                    ...
                                    while(basisj[lketinc-1] != -1):
                                        ...
                                        for mp in range(min(lbra,lket)+1):
                                            ...
                                            mybondint[mp] = univscale(i,j,lbra,lket,mp,magr,tabs,tabh,\
                                                  sspl,hspl,iglMap,lentabint,tabr,tabh,hcut,scut,"H")
                                            myoverlapint[mp] = univscale(i,j,lbra,lket,mp,magr,tabs,tabh,\
                                                  sspl,hspl,iglMap,lentabint,tabr,tabh,hcut,scut,"S")
                                        ...
                                        for mket in range(-lket,lket+1):
                                            ...
                                            ham[ibra-1,iket-1] = ham[ibra-1,iket-1] + myangfactor * \
                                                  mybondint[0]
                                            smat[ibra-1,iket-1] = smat[ibra-1,iket-1] + myangfactor * \
                                                  myoverlapint[0]
                                            #Everything else
                                            count = count + 1
            ...
            for l2 in range(lMax+1):
             ...
             # Easier case first ATELE(I) = ATELE(J)
                    ...
                    if(atele[i] == atele[j]):
                        ...
                        if(l1 == l2):
                            for ic in range(noint):
                                if((( (atele[i] == ele1[ic]) and (atele[j] == ele2[ic])) or \
                                        ( (atele[i] == ele2[ic]) and (atele[j] == ele1[ic]) )) and \
                                        (igltype == btype[ic]) ):
...
def build_integralMap_parPool(noint,btype,atele,ele1,ele2,nats,verbose,parallel=1):
    ...
    if(parallel > 1):
        ...
        results = [pool.apply_async( build_integralMap_I, \
                [noint,btype,atele,ele1,ele2,nats,lMax,mpMax,btypeInt,val]) for val in range(0,nats)]
        for idx, val in enumerate(results):
            iglMap[:,:,:,:,idx] = val.get()
        pool.close()
    elif(parallel == 1):
        for val in range(nats):
            iglMap[:,:,:,:,val] = build_integralMap_I(noint,btype,atele,ele1,ele2,nats,lMax,mpMax,btypeInt,val)
...
def univscale(i,j,l1,l2,mp,r,tabs,tbh,sspl,hspl,iglMap,lentabint,tabr,tabh,hcut,scut,whichint):
    ...
    if(whichint == "H"):
        univscale = sa*tabh[klo,myIntegral] + sb*tabh[khi,myIntegral] + \
                ...
                (sb*sb*sb - sb)*hspl[khi,myIntegral])*(dx*dx/6.0)
        if(r > hcut[myIntegral]): univscale = 0.0
    else:
        univscale = sa*tabs[klo,myIntegral] + sb*tabs[khi,myIntegral] +\
                ((sa*sa*sa - sa)*sspl[klo,myIntegral] + \
                (sb*sb*sb - sb)*sspl[khi,myIntegral])*(dx*dx/6.0)
        if(r > scut[myIntegral]): univscale = 0.0

    if((l1 > l2) and ((l1 + l2 % 2) != 0)): univscale = -1.0*univscale  
...
def angfactor(lbra,lket,mbra,mket,mp,alpha,cosbeta):
    ...
    if(mp == 0):
        ...
        angfactor = 2.0*(am(mbra,alpha)*wig_lbra_mbra_0) * \
        ...
        angfactor = am(mbra,alpha)*(((-1.0)**mp) * \
                wig_lbra_mbra_mp + wig_lbra_mbra_negmp) * \
                am(mket,alpha)*(((-1.0)**mp) * \
                wig_lket_mket_mp + wig_lket_mket_negmp)

        if(mbra == 0):
            mytlmmp = 0.0 
        else:
            mytlmmp = bm(mbra,alpha)*(((-1.0)**mp) * \
                    wig_lbra_mbra_mp - wig_lbra_mbra_negmp)

        if(mket == 0):
            mytlmmp = 0.0 
        else:
            mytlmmp = mytlmmp*bm(mket,alpha)*(((-1.0)**mp) * \
                    wig_lket_mket_mp - wig_lket_mket_negmp)
...
def wignerd(l,m,mp,cosbeta):
    ...
    if(abs(mp) > l):
        ...
        for k in range(max(0,-m -mp),min(l-m,l-mp)+1):
            ...
            wignerd = wignerd + ((-1.0)**k) * \
                    ((1.0 - cosbeta)**power1) * ((1.0 + cosbeta)**power2) / \
                    float(factorial(k) * factorial(l - m - k) * \
...
def read_bondints(electronsFileName,bondintsFileName,verbose):
    ...
    if(verbose): print("In read_bondints ...")
    ...
    for i in range(noint):
        ...
        for j in range(1,n-1):
            ...
            hspl[j,i] = (sig - 1.0)/p
            paramU[j] = (6.0*((tabh[j+1,i] - tabh[j,i]) /  \
    ...
    return(noelem,ele,ele1,ele2,basis,hes,hep,hed,hef,hcut,scut,noint,\
            btype,tabh,tabs,sspl,hspl,lentabint,tabr,atocc,mass,hubbardu)
    ...
    for i in range(nats):
        for j in range(nats):
            ...
            for k in range(noint):
                if((atele[i] == ele1[k] and \
                atele[j] == ele2[k]) or  \
                (atele[j] == ele1[k] and \
                atele[i] == ele2[k])):
                    if(hcut[k] > rcuttb): rcuttb = hcut[k]
                    if(scut[k] > rcuttb): rcuttb = scut[k]
...
def get_cutoffList_par(nats,atele,ele1,ele2,hcut,scut,noint,verbose):
    ...
    cutoffList = Parallel(n_jobs=12)(delayed( \
            get_cutoffList_I)(nats,atele,ele1,ele2,hcut,scut,noint,i) for i in range(nats))
...
def get_cutoffList_parPool(nats,atele,ele1,ele2,hcut,scut,noint,verbose,parallel=1):
    ...
    if(parallel > 1):
        pool = Pool(processes=parallel)
        results = [pool.apply_async(get_cutoffList_I_vect, \
                [nats,atele,ele1,ele2,hcut,scut,noint,val]) for val in range(0,nats)]
        for idx, val in enumerate(results):
...
def fermi_exp(hmat,zmat,nocc,T):
  ...
  while (OccErr > 1e-12):     # Adjusts the chemical potential for a given Fermi-Dirac
      ...
      for i in range(hdim):
          ...
          if tmp > 100:
             tmp = 100.
          focc[i] = 1./(np.exp(tmp)+1.) 
      occ = np.sum(focc)
      docc = np.sum(beta*focc*(1.-focc))
  ...
  if (docc > 1e-14):
      focc = focc + ((nocc-occ)/docc)*beta*focc*(1.-focc)
  ...
  for i in range(hdim):
      if focc[i] > 1e-14 and focc[i] < 1.0-1e-14:
          entropy = entropy - kB*(focc[i]*np.log(focc[i]) + (1.-focc[i])*np.log(1.-focc[i]))
